...
class DashboardView(View):
    def get(self, request):
        ...
        incomes_by_day = (
            Income.objects
            .filter(date__range=[last_30, today])
        ...
        chart2_incomes  = [
            float(Income.objects.filter(business=b).aggregate(t=Sum('amount'))['t'] or 0)
            for b in businesses
        ]
        chart2_expenses = [
            float(Expense.objects.filter(business=b).aggregate(t=Sum('amount'))['t'] or 0)
            for b in businesses
        ]
        ...
        today_purchases = Purchase.objects.filter(purchase_date=today).aggregate(t=Sum('amount'))['t'] or 0
